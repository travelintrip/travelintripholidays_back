...
export const editHomeData = async (req, res) => {
  try {
    const {
      ...
      LoginBanner,
WalletRechargeBanner,
MyLeadsBanner,
BuyLeadBanner,
DashboardScreenBanner,
WalletPageBanner,
AnalyticsPageBanner,
MyAccountBanner,
...
export const editOrderAdmin = async (req, res) => {
  try {
    ...
    const mailOptions = {
      ...
      subject: `cayroshop.com Order ${status === "0"
        ? "cancel"
        : status === "1"
          ? "Placed"
          : status === "2"
    ...
    "> Order Id : #${order.orderId} </h5>
          ...
          <svg xmlns="http://www.w3.org/2000/svg" width="100" height="100" viewBox="0 0 24 24" fill="none" stroke="#47ca00" stroke-width="2" stroke-linecap="square" stroke-linejoin="arcs"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"></path><polyline points="22 4 12 14.01 9 11.01"></polyline></svg>
           <h2 style="color:black;"> Your Order Is ${status === "1"
          ? "Placed"
          : status === "2"
            ? "Accepted"
            : status === "3"
    ...
    as soon as your order ${status === "1"
          ? "Placed"
          : status === "2"
            ? "Accepted"
            : status === "3"
          ...
          <div class="modal-footer">
            ...
            <a href="https://cayroshop.com/account/order/${_id}/${updatedOrder._id
        }" style="
        background: green;
        color: white;
        padding: 10px;
...
export const getAllUserAdmin_old = async (req, res) => {
  try {
    ...
    const Recharge = req.query.recharge || "";
    const fwalletSort = req.query.fwallet !== undefined ? parseInt(req.query.fwallet) : null;
    ...
    if (verified.length > 0) {
      query.verified = { $in: verified };
    }

    if (status.length === 0) {
      query.status = { $in: [0, 1] };
    } else {
      query.status = { $in: Number(status) };
    }
    ...
    // Determine sorting order for fwallet
    let sortQuery = { _id: -1 }; // Default sorting by latest users
    if (fwalletSort === 1) {
      sortQuery = { fwallet: -1 }; // High to Low
    } else if (fwalletSort === 0) {
      sortQuery = { fwallet: 1 }; // Low to High
    }else if (Recharge === 1) {
      sortQuery = { LastRecharge: -1 };// High to Low
    }else if(Recharge === 0) {
      sortQuery = { LastRecharge: 1 };// Low to High
    }else{
      sortQuery = { _id: -1 }
    }
    ...
    return res.status(200).send({
      ...
      totalPages: Math.ceil(totalUser / limit),
      success: true,
      users: encrypt(users, process.env.APIKEY),
    });
  } catch (error) {
    return res.status(500).send({
...
export const getAllUserAdmin = async (req, res) => {
  try {
    ...
    let sortQuery = { _id: -1 }; // Default sorting by latest users
    if (fwalletSort === 1) {
      sortQuery = { wallet: -1 }; // High to Low
    } else if (fwalletSort === 0) {
      sortQuery = { wallet: 1 }; // Low to High
    }else if (Recharge === "1") {
      sortQuery = { LastRecharge: -1 };// High to Low
    }else if(Recharge === "0") {
      sortQuery = { LastRecharge: 1 };// Low to High
    ...
    if (req.query.type === "0" || req.query.type === 0) {
      // Calculate vendor states only if type === 0
    ...
    return res.status(200).send({
      ...
      success: true,
      users: encrypt(users, process.env.APIKEY),
      extraFilter, // Include the extraFilter in the response
    });
...
export const GetLeadByIDAdmin = async (req, res) => {
  try {
    const { id } = req.params;
    ...
    return res.status(400).json({
      message: `Error while getting lead: ${error.message}`,
      success: false,
      error: error.message,
    });
  }
...
export const updateLeadAdmin = async (req, res) => {
  const { id } = req.params;
  ...
  try {
    ...
    // Update the zone
    await LeadModel.findByIdAndUpdate(id, updateFields, {
      new: true,
    });
    ...
    console.error("Error while update Leads:", error);
    return res.status(400).send({
      success: false,
      message: "Error while update Leads",
      error,
    });
  }
};
...
export const getAllPaymentsAdmin = async (req, res) => {
  try {
    ...
    const totalData = await transactionModel.countDocuments(query); // Count total documents matching the query
    ...
    const data = await transactionModel
      ...
      .populate({
        ...
        select: "phone username wallet", // Only select the phone and name fields from the User model
      })
      .lean(); // Convert documents to plain JavaScript objects
